# Enviar correo con la API v3 de Sendinblue

## Enviar con Postman

Para enviar correos mediante Postman, Se debe enviar una peticion HTTP con las siguientes características.
...
(La API-KEY se obtiene en <https://account.sendinblue.com/advanced/api>)
...
```json
{
    ...
    "htmlContent": "<html><head></head><body><p>¡Hola!</p>Este es mi primer correo transaccional enviado con Sendinblue.</p></body></html>"
}
```

Extraido de: <https://developers.sendinblue.com/docs/send-a-transactional-email>

## Adjuntar archivo

Si se quiere adjuntar un archivo, existen dos maneras:

- Adjuntar mediante URL a un archivo público
- Adjuntar el archivo en `base64`

Si se quiere adjuntar usando URL, se debe agregar el siguiente atributo al JSON mostrado en las secciones anteriores:

```json
{
    "attachment": [
        {
            "name": "ArchivoAdjuntoMedianteURL.jpg",
            "url": "https://static3.abc.es/media/201101/26/inteligencia1--644x362.jpg"
        }
    ]
}
```

Si se quiere adjuntar usando Base64, se debe agregar el siguiente atributo al JSON mostrado en las secciones anteriores:

**Nota:** El string en `base64` NO debe comenzar con `data:image/png;base64,`
...
Si se quiere utilizar un template, se deben agregar los siguientes atributos al JSON:

```json
{
    "templateId": 8,
    "params": {
        "claveParametro1": "valor del parametro 1",
        "claveParametro2": "valor del parametro 2"
    }
}
```
...
{
    "sender": {
        "email": "reservas@uruguaybus.uy",
        "name": "UruguayBus"
    },
    "to": [
        {
            "email": "jjap96@gmail.com",
            "name": "Juan José"
        }
    ],
